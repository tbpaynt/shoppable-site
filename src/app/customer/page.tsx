...
export default function CustomerPanel() {
  ...
  if (status === "loading") {
    return (
      <div className="min-h-screen flex items-center justify-center text-2xl text-white" style={{background: 'radial-gradient(ellipse at bottom, #1b2735 0%, #090a0f 100%)'}}>
        Loading...
      </div>
    );
  }
  ...
  if (status !== "authenticated" || !session.user?.email) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center" style={{background: 'radial-gradient(ellipse at bottom, #1b2735 0%, #090a0f 100%)'}}>
        <h1 className="text-3xl font-bold mb-4 text-white">Please sign in to view your orders</h1>
  ...
  return (
    <div className="min-h-screen py-8" style={{background: 'radial-gradient(ellipse at bottom, #1b2735 0%, #090a0f 100%)'}}>
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-white mb-8">Deals You Snagged</h1>
...
function PurchaseHistory({ userEmail }: { userEmail: string }) {
  ...
  const toggleEmailNotifications = async (orderId: string) => {
    try {
      ...
      if (!response.ok) {
        console.error('Error updating notification preferences:', result);
        alert(`Failed to update notification preferences: ${result.error || 'Unknown error'}`);
        return;
      }
      ...
      console.log(`Email notifications ${newValue ? 'enabled' : 'disabled'} for order ${orderId}`);
    } catch (error: any) {
      console.error('Error updating notification preferences:', error);
      alert(`An unexpected error occurred: ${error?.message || error?.toString() || 'Unknown error'}`);
    }
  };
    ...
    <div className="bg-gray-800 rounded-lg shadow">
      <div className="p-6">
        ...
        {orders.map((order) => {
          ...
          return (
            <div key={order.id} className="bg-gray-700 p-4 rounded-lg border border-gray-600 shadow-sm">
